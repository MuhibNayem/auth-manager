...
Part of **tessera 2.0** — see the root `README.md` for the overall
package (features, extras, security configuration) and the note about the
planned rename away from the "Tessera" name.

This guide covers the enterprise SSO subsystems rebuilt during the 2.0
remediation against `docs/CONTRACTS.md`:

- **SAML 2.0 Service Provider** — install with `pip install "tessera[saml]"`
  (lxml, xmlsec, python3-saml).
- **OpenID Connect client** — install with `pip install "tessera[oidc]"`
  (no dependencies beyond the core; discovery + PKCE included).

### SAML 2.0 Service Provider

```python
from tessera.saml import SAMLManager, SAMLConfig
...
```python
from tessera.oidc import OIDCManager, OIDCConfig
...
```bash
psql -d your_database -f tessera/migration/saml_oidc_tables.sql
```
