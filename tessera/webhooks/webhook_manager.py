...
- **Signatures**: HMAC-SHA256 over ``f"{timestamp}.{canonical_json_bytes}"``;
  headers ``X-Tessera-Signature: sha256=<hex>``, ``X-Tessera-Timestamp``
  (unix seconds), ``X-Tessera-Event``, ``X-Tessera-Delivery-Id``. The receiver
...
- Persistence uses the §4 db webhook methods; the pending-queue lives in
  the cache list ``tessera:webhook:queue`` (§3.1).
"""
...
logger = logging.getLogger("tessera.webhooks")
...
WEBHOOK_QUEUE_KEY = "tessera:webhook:queue"

#: Cache key prefix for the delivery-id replay ledger (receiver side).
DELIVERY_SEEN_KEY_TEMPLATE = "tessera:webhook:seen:{delivery_id}"
...
SIGNATURE_HEADER = "X-Tessera-Signature"
TIMESTAMP_HEADER = "X-Tessera-Timestamp"
EVENT_HEADER = "X-Tessera-Event"
DELIVERY_ID_HEADER = "X-Tessera-Delivery-Id"
        ...
        try:
            ...
            response = await self._client().post(
                ...
                headers={
                    ...
                    "User-Agent": "Tessera-Webhooks/2.0",
                },
