...
- Legacy password hashes (``user["password_algorithm"]`` set) are verified
  ...
  :func:`tessera.migration.verify_and_upgrade_legacy_hash`.
- The MFA gate requires a valid TOTP code; attempt limiting lives in
  :class:`~tessera.mfa.mfa_setup.MFAAuthManager`.
- Token pairs come from :class:`JWTTokenManager`; refresh tokens rotate
  through the §3.1 cache ledger (``tessera:refresh:{jti}`` deleted on use).
...
logger = logging.getLogger("tessera.core")

__all__ = ["TraditionalAuthManager", "CognitoAuthManager", "SocialAuthManager"]

#: §3.1 cache key schema.
REFRESH_LEDGER_KEY_TEMPLATE = "tessera:refresh:{jti}"
ACCESS_LEDGER_KEY_TEMPLATE = "tessera:access:{jti}"
TOKENPAIR_KEY_TEMPLATE = "tessera:tokenpair:{user_id}"
SOCIAL_TOKEN_KEY_TEMPLATE = "tessera:socialtoken:{user_id}:{provider}"
...
class TraditionalAuthManager:
    ...
    async def _authenticate_password(
        ...
        if algorithm and algorithm.lower() not in ("bcrypt", "argon2"):
            from tessera.migration import verify_and_upgrade_legacy_hash
...
class SocialAuthManager:
    ...
    Args:
        ...
        github_manager / apple_manager / facebook_manager / google_manager:
            Provider clients (see ``tessera.social``).
    """
