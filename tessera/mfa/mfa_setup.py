...
- Verification attempts (both confirmation and login) are rate limited with
  cache counters (§3.1 ``tessera:ratelimit:mfa:{user_id}``).
...
from tessera.errors import AuthenticationError, ConfigError, RateLimitError

logger = logging.getLogger("tessera.mfa")

__all__ = ["MFAAuthManager"]

#: Cache key holding a pending (unconfirmed) TOTP secret (§3.1 style).
PENDING_SECRET_KEY_TEMPLATE = "tessera:mfa:pending:{user_id}"

#: Cache key for MFA attempt counters (§3.1 ratelimit namespace).
MFA_RATE_LIMIT_KEY_TEMPLATE = "tessera:ratelimit:mfa:{user_id}"
...
def _lazy_pyotp():
    ...
    try:
        ...
        raise ImportError(
            "MFA requires the 'pyotp' package; install tessera with "
