"""Configuration model for the Tessera package.
...
password_security). :meth:`AuthConfig.from_env` reads ``TESSERA_*`` environment
variables (names preserved from the legacy implementation); new fields add
new ``TESSERA_*`` variables following the same convention.
...
logger = logging.getLogger("tessera.config")
...
@dataclass
class AuthConfig:
    """Main configuration for the Tessera package (CONTRACTS.md §2).
    ...
    def from_env(cls) -> "AuthConfig":
        ...
        Legacy ``TESSERA_*`` variable names are preserved; new canonical fields
        add ``TESSERA_*`` variables following the same convention.
        """
        return cls(
            env=_env_str("TESSERA_ENV", "development") or "development",
            jwt_secret=_env_str("TESSERA_JWT_SECRET"),
            jwt_algorithm=_env_str("TESSERA_JWT_ALGORITHM", "HS256") or "HS256",
            access_token_ttl_seconds=_env_int("TESSERA_TOKEN_EXPIRATION", 3600),
            refresh_token_ttl_days=_env_int("TESSERA_REFRESH_TOKEN_TTL_DAYS", 7),
            jwt_issuer=_env_str("TESSERA_JWT_ISSUER"),
            jwt_audience=_env_str("TESSERA_JWT_AUDIENCE"),
            jwt_clock_skew_seconds=_env_int("TESSERA_JWT_CLOCK_SKEW_SECONDS", 30),
            session_expiry_seconds=_env_int("TESSERA_SESSION_EXPIRY_SECONDS", 86400 * 7),
            max_concurrent_sessions=_env_int("TESSERA_MAX_CONCURRENT_SESSIONS", 5),
            revoke_sessions_on_password_change=_env_bool(
                "TESSERA_REVOKE_SESSIONS_ON_PASSWORD_CHANGE", True
            ),
            rate_limit_enabled=_env_bool("TESSERA_RATE_LIMIT_ENABLED", True),
            rate_limit_max_attempts=_env_int("TESSERA_RATE_LIMIT_MAX_ATTEMPTS", 5),
            rate_limit_window_seconds=_env_int("TESSERA_RATE_LIMIT_WINDOW_SECONDS", 300),
            account_lockout_duration_seconds=_env_int(
                "TESSERA_ACCOUNT_LOCKOUT_DURATION_SECONDS", 900
            ),
            mfa_required=_env_bool("TESSERA_MFA_REQUIRED", False),
            password_hash_algorithm=_env_str("TESSERA_PASSWORD_HASH_ALGORITHM", "bcrypt") or "bcrypt",
            bcrypt_rounds=_env_int("TESSERA_BCRYPT_ROUNDS", 12),
            reset_token_ttl_seconds=_env_int("TESSERA_RESET_TOKEN_TTL_SECONDS", 900),
            base_url=_env_str("TESSERA_BASE_URL", "http://localhost:8000") or "http://localhost:8000",
            magic_link_ttl_seconds=_env_int("TESSERA_MAGIC_LINK_TTL_SECONDS", 600),
            default_redirect_url=_env_str("TESSERA_DEFAULT_REDIRECT_URL"),
            auto_create_users=_env_bool("TESSERA_AUTO_CREATE_USERS", False),
            rp_id=_env_str("TESSERA_RP_ID"),
            rp_name=_env_str("TESSERA_RP_NAME", "Tessera") or "Tessera",
            email_enabled=_env_bool("TESSERA_EMAIL_ENABLED", False),
            email_provider=_env_str("TESSERA_EMAIL_PROVIDER", "mailjet") or "mailjet",
            ...
            database=DatabaseConfig(
                db_type=_env_str("TESSERA_DB_TYPE", "sql") or "sql",
                connection_string=_env_str("TESSERA_DB_URL", "") or "",
                db_name=_env_str("TESSERA_DB_NAME"),
                collection_name=_env_str("TESSERA_DB_COLLECTION"),
            ),
            cache=CacheConfig(
                enabled=_env_bool("TESSERA_CACHE_ENABLED", True),
                redis_url=_env_str("TESSERA_REDIS_URL", "redis://localhost:6379")
                or "redis://localhost:6379",
                token_expiration=_env_int("TESSERA_TOKEN_EXPIRATION", 3600),
                refresh_token_expiration=_env_int("TESSERA_REFRESH_TOKEN_EXPIRATION", 604800),
            ),
            ...
            cognito=CognitoConfig(
                enabled=_env_bool("TESSERA_COGNITO_ENABLED", False),
                region_name=_env_str("AWS_REGION"),
                user_pool_id=_env_str("COGNITO_USER_POOL_ID"),
                app_client_id=_env_str("COGNITO_APP_CLIENT_ID"),
            ),
            sms=SMSConfig(
                enabled=_env_bool("TESSERA_SMS_ENABLED", False),
                provider=_env_str("TESSERA_SMS_PROVIDER", "twilio") or "twilio",
                ...
                code_length=_env_int("TESSERA_SMS_CODE_LENGTH", 6),
                expiration_seconds=_env_int("TESSERA_SMS_CODE_EXPIRATION", 300),
                max_attempts=_env_int("TESSERA_SMS_MAX_ATTEMPTS", 3),
            ),
            bot_protection=BotProtectionConfig(
                enabled=_env_bool("TESSERA_BOT_PROTECTION_ENABLED", True),
                provider=_env_str("TESSERA_CAPTCHA_PROVIDER", "hcaptcha") or "hcaptcha",
                ...
                enable_rate_limiting=_env_bool("TESSERA_RATE_LIMIT_ENABLED", True),
                max_requests_per_minute=_env_int("TESSERA_MAX_REQUESTS_PER_MINUTE", 10),
                max_requests_per_hour=_env_int("TESSERA_MAX_REQUESTS_PER_HOUR", 100),
            ),
            password_security=PasswordSecurityConfig(
                check_breached=_env_bool("TESSERA_CHECK_BREACHED_PASSWORDS", True),
                hibp_api_key=_env_str("HIBP_API_KEY"),
                min_length=_env_int("TESSERA_PASSWORD_MIN_LENGTH", 12),
                require_uppercase=_env_bool("TESSERA_PASSWORD_REQUIRE_UPPERCASE", True),
                require_lowercase=_env_bool("TESSERA_PASSWORD_REQUIRE_LOWERCASE", True),
                require_numbers=_env_bool("TESSERA_PASSWORD_REQUIRE_NUMBERS", True),
                require_special_chars=_env_bool("TESSERA_PASSWORD_REQUIRE_SPECIAL_CHARS", True),
                disallow_common_passwords=_env_bool("TESSERA_PASSWORD_DISALLOW_COMMON", True),
                disallow_sequential_chars=_env_bool("TESSERA_PASSWORD_DISALLOW_SEQUENTIAL", True),
                disallow_repeated_chars=_env_bool("TESSERA_PASSWORD_DISALLOW_REPEATED", True),
                max_repeated_chars=_env_int("TESSERA_PASSWORD_MAX_REPEATED", 3),
                disallow_username_in_password=_env_bool("TESSERA_PASSWORD_DISALLOW_USERNAME", True),
            ),
        )
    ...
    def validate(self) -> bool:
        ...
        if not self.jwt_secret:
            errors.append(
                "jwt_secret must be set (env TESSERA_JWT_SECRET); there is no default"
            )
        ...
        if self.database.db_type not in VALID_DB_TYPES:
            ...
            errors.append(
                ...
                f"{self.database.db_type!r} (env TESSERA_DB_URL)"
            )
