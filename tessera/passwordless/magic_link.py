...
- Tokens are ``secrets.token_urlsafe(32)`` (256 bits of entropy) and live
  only in the cache under ``tessera:magiclink:{token}`` with
...
- Sending is rate limited via cache counters
  (``tessera:ratelimit:magiclink:{email}``).
...
logger = logging.getLogger("tessera.passwordless.magic_link")

__all__ = ["MagicLinkManager"]

#: §3.1 cache key schema.
MAGIC_LINK_KEY_TEMPLATE = "tessera:magiclink:{token}"
SEND_RATE_KEY_TEMPLATE = "tessera:ratelimit:magiclink:{email}"
