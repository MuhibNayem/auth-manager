...
- :func:`get_v2_principal` guards every ``/admin/v2`` route and accepts
  either an admin JWT or a hashed ``tessera_ak_...`` API key validated
  against the db (§8).
- TesseraError → HTTP status mapping (§1) installed by
...
logger = logging.getLogger("tessera.admin.deps")
...
def tessera_error_status(error: TesseraError) -> int:
    """Map an TesseraError subclass to its HTTP status code (§1)."""
...
def register_error_handlers(app: Any) -> None:
    """Install TesseraError/ValueError/PermissionError handlers on a FastAPI app."""

    async def _tessera_error_handler(request: Request, exc: TesseraError):
        from fastapi.responses import JSONResponse

        status_code = tessera_error_status(exc)
        headers = {}
    ...
    app.add_exception_handler(TesseraError, _tessera_error_handler)
...
class AdminDependencies:
    ...
    :func:`tessera.admin.dashboard_api.install_admin_api`.
    """
...
async def get_v2_principal(
    ...
    Accepts:
    - ``tessera_ak_...`` API keys — sha256-hashed and validated against the
