...
@click.pass_context
def cli(ctx, version):
    """Tessera CLI — authentication infrastructure tooling.
    ...
    """
    if version:
        console.print(f"[bold blue]Tessera CLI[/bold blue] v{__version__}")
        return

    if ctx.invoked_subcommand is None:
        console.print(Panel.fit(
            "[bold blue]Tessera CLI[/bold blue]\n\n"
            "Quick Start:\n"
            "  [green]tessera init[/green]       Initialize a new project\n"
            "  [green]tessera dev[/green]        Start development server (127.0.0.1)\n"
            "  [green]tessera doctor[/green]     Run health checks\n"
            "  [green]tessera migrate[/green]    Run database migrations\n"
            "  [green]tessera deploy[/green]     Docker build / IaC artifact generation\n\n"
            "Use [bold]tessera <command> --help[/bold] for full usage.",
            title="Welcome",
            border_style="blue",
        ))
