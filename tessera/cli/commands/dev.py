"""
Tessera Dev Command - Development Server with Hot Reload
...
async def _run_dev_server(host: str, port: int, reload: bool, https: bool, 
    ...
    console.print(Panel.fit(
        "[bold blue]🛡️  Tessera Development Server[/bold blue]\n\n"
    ...
    os.environ['TESSERA_ENV'] = 'development'
    os.environ['TESSERA_DEBUG'] = '1' if debug else '0'
    
    if mock_services:
        os.environ['TESSERA_MOCK_EMAIL'] = '1'
        os.environ['TESSERA_MOCK_SMS'] = '1'
        console.print("[dim]✓ Mock email/SMS services enabled[/dim]")
    
    # Generate self-signed cert if HTTPS enabled
    if https:
        cert_dir = Path.cwd() / '.tessera' / 'certs'
...
async def _generate_cert(cert_file: Path, key_file: Path):
    ...
    try:
        ...
        subject = issuer = x509.Name([
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Tessera Development"),
