"""``tessera doctor`` — honest health checks and diagnostics (CONTRACTS.md §9).
...
async def _check_database(config) -> Tuple[bool, str]:
    ...
    if db_type == "memory":
        ...
        return False, f"db_type '{db_type}' but TESSERA_DB_URL is not set"
    else:
...
async def _check_cache(config) -> Tuple[bool, str]:
    ...
    try:
        from tessera.cache import RedisCache
...
async def collect_checks() -> Tuple[List[Tuple[str, bool, str]], int]:
    ...
    try:
        ...
        rows.append(("Core import", True, f"tessera {tessera.__version__}"))
    except Exception as exc:
    ...
    # 4. Config validation
    try:
        from tessera.cli.utils import load_auth_config
    ...
    if config_ok:
        try:
            from tessera.cli.utils import load_auth_config as _lc
...
def doctor_cmd(verbose: bool):
    ...
    table = Table(title="Tessera Health Check", show_header=True)
