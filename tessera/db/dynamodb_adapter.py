...
from :mod:`tessera.db.enterprise_utils`.
...
logger = logging.getLogger("tessera.db.dynamodb")
...
class DynamoDBAdapter(AbstractDatabase, ObservabilityMixin):
    ...
    Args:
        config: A :class:`~tessera.config.DatabaseConfig` (its
    ...
    def __init__(self, config: Any = None) -> None:
        ...
        if isinstance(config, dict):
            ...
            cfg = {
                ...
                "table_prefix": getattr(config, "table_prefix", "tessera_"),
                "region": getattr(config, "region", None),
            }
        self.config = cfg
        self.table_prefix = cfg.get("table_prefix", "tessera_")
