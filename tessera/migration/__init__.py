...
logger = logging.getLogger("tessera.migration")
...
class BaseImporter(ABC):
    ...
    async def _migrate_user(
        ...
        tessera_user = {k: v for k, v in tessera_user.items() if v is not None}
        ...
        if "password_hash" in user_data and user_data["password_hash"]:
            if strategy == "full":
                ...
                if translated:
                    tessera_user["hashed_password"] = translated
                    tessera_user["password_needs_rehash"] = True
                else:
                    tessera_user["password_reset_required"] = True
            else:  # lazy-password-migration
                tessera_user["legacy_password_hash"] = {
                    "hash": user_data["password_hash"],
                    "algorithm": user_data.get("password_algorithm", "unknown"),
                    "provider": self.provider_name,
                    "scrypt_params": user_data.get("scrypt_params"),
                }
                tessera_user["password_algorithm"] = user_data.get(
                    "password_algorithm", "unknown"
                )

        return await self.db.create_user(tessera_user)
