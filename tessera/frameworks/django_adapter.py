...
- Provides ``require_auth``, ``require_role``, ``rate_limit`` and
  ...
  adapters.
- Authenticated users are attached to ``request.tessera_user`` (dict).
  ``request.user`` is NEVER clobbered.
- ``DjangoAuthMiddleware`` resolves bearer tokens into ``request.tessera_user``
...
logger = logging.getLogger("tessera.frameworks.django")
...
def _status_for_error(exc: TesseraError) -> int:
    """HTTP status for an TesseraError (§1 mapping)."""
    if isinstance(exc, (AuthenticationError, TokenError)):
        return 401
    if isinstance(exc, RateLimitError):
        return 429
    from tessera.errors import AuthorizationError, NotFoundError, ProviderError
...
class DjangoAuth:
    ...
    Usage::
        ...
        def protected(request):
            return JsonResponse({"user": request.tessera_user})
    ...
    def _authenticate(self, token: str) -> Dict[str, Any]:
        ...
        Raises TesseraError subclasses on failure (callers map to responses).
        """
    ...
    def require_auth(self, view_func: Callable) -> Callable:
        """Decorator requiring authentication; sets ``request.tessera_user``.
    ...
    def require_role(self, *roles: str) -> Callable:
        ...
        def decorator(view_func: Callable) -> Callable:
            @wraps(view_func)
            def wrapper(request, *args, **kwargs):
                user = getattr(request, "tessera_user", None)
                if user is None:
                    return self._json_response(
                        {
    ...
    def optional_auth(self, view_func: Callable) -> Callable:
        """Decorator setting ``request.tessera_user`` when a valid token exists."""

        @wraps(view_func)
        def wrapper(request, *args, **kwargs):
            request.tessera_user = None
            token = self._token_from_request(request)
            if token:
                try:
                    request.tessera_user = self._authenticate(token)
                except TesseraError as exc:
...
class DjangoAuthMiddleware:
    """Middleware resolving bearer tokens into ``request.tessera_user``.
        ...
        if header.startswith("Bearer "):
            if auth_manager is None:
                logger.error(
                    ...
                    "tessera.frameworks.django_adapter.set_auth_manager()"
                )
