<script lang="ts">
	...
	// Svelte 4-compatible component (also compiles under Svelte 5).
	...
	const dispatch = createEventDispatcher<{
		success: { user: unknown; token: string };
		error: { message: string };
	}>();

	// --- API location (fully configurable) ----------------------------------
	// Point `apiBaseUrl` at the auth backend origin (default: current origin)
	// and `loginPath` at the login route it serves. The request goes to
	// `${apiBaseUrl}${loginPath}`.
	export let apiBaseUrl = '';
	export let loginPath = '/api/auth/login';

	// Social buttons (rendered only when `showSocial` is true) navigate to
	...
	// Optional links: each renders only when a URL is provided, so the
	// component never asserts routes the host app does not implement.
	export let magicLinkUrl = '';
	export let forgotPasswordUrl = '';
	export let signUpUrl = '';
	export let showMagicLink = true;

	// Redirect after successful login; empty string disables auto-redirect
	// (the parent can handle the `success` event instead).
	...
	$: actionUrl = `${apiBaseUrl}${loginPath}`;
...
<div class="tessera-login-container">
	<div class="tessera-card">
		...
		{#if success}
			<div class="tessera-success">
				<p>{redirectUrl ? 'Success! Redirecting...' : 'Success! You are signed in.'}</p>
			</div>
		{:else}
			<form on:submit={handleSubmit} class="tessera-form">
				{#if error}
					<div class="tessera-error">{error}</div>
				{/if}
				...
				<div class="tessera-field">
					<div class="tessera-label-row">
						...
						{#if forgotPasswordUrl}
							<a href={forgotPasswordUrl} class="tessera-link">Forgot?</a>
						{/if}
					</div>
					<input
						type="password"
				...
				<button type="submit" class="tessera-button" disabled={loading}>
					{#if loading}
						<span class="tessera-spinner"></span> Signing in...
					{:else}
						{submitLabel}
					{/if}
				</button>
			...
			{#if showSocial}
				<div class="tessera-social-grid">
					<button type="button" class="tessera-social-btn google" on:click={() => handleSocialLogin('google')}>
					...
					<button type="button" class="tessera-social-btn github" on:click={() => handleSocialLogin('github')}>
