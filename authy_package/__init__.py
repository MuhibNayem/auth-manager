"""Authy Package — authentication building blocks (CONTRACTS.md).
...
- Every other submodule is probed with a guarded import at package init:
  ...
  ``__all__``. ``import authy_package`` therefore succeeds with zero
...
logger = logging.getLogger("authy")
...
#: name -> (submodule path relative to authy_package, attribute name).
...
def _probe_submodules() -> None:
    ...
    for flag_prefix, module_path in _SUBMODULE_PROBES.items():
        try:
            ...
            available = False
            logger.debug("authy_package.%s unavailable: %s", module_path, exc)
...
def __getattr__(name: str) -> Any:
    ...
    if name in _LAZY_EXPORTS:
        ...
        try:
            ...
            raise ImportError(
                f"authy_package.{name} is unavailable: failed to import "
                f"authy_package.{module_path}: {exc}"
            ) from exc
        try:
            value = getattr(module, attribute)
        except AttributeError as exc:
            raise ImportError(
                f"authy_package.{name} is unavailable: "
                f"authy_package.{module_path} has no attribute {attribute!r}"
            ) from exc
