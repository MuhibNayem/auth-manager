...
from authy_package.errors import AuthenticationError, ConfigError, ProviderError

logger = logging.getLogger("authy.oidc")

__all__ = ["OIDCConfig", "OIDCManager"]

#: §3.1-style cache key templates.
STATE_KEY_TEMPLATE = "authy:oauth:state:{state}"
NONCE_KEY_TEMPLATE = "authy:oauth:nonce:{nonce}"
TOKEN_CACHE_KEY_TEMPLATE = "authy:oidc:tokens:{token_hash}"
#: db settings fallback key for state storage.
STATE_SETTING_TEMPLATE = "authy_oidc_state:{state}"
