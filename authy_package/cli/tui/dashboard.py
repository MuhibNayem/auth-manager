"""Authy TUI dashboard — single honest snapshot render (CONTRACTS.md §9).
...
def make_header() -> Panel:
    ...
    text = Text.from_markup("[bold blue]Authy Dashboard[/bold blue]")
    text.append(
...
def make_stats_panel(db: Optional[Any]) -> Panel:
    ...
    if db is None:
        ...
        table.add_row("Hint", "set AUTHY_DB_TYPE / AUTHY_DB_URL")
...
def make_footer() -> Panel:
    ...
    return Panel(
        "[dim]Single snapshot — re-run 'authy dashboard' to refresh.[/dim]",
        border_style="dim",
    )
...
async def fetch_snapshot() -> Optional[_Snapshot]:
    ...
    from authy_package.cli.utils import get_connected_db
...
async def run_dashboard() -> None:
    ...
    layout["active-sessions"].update(
        Panel(
            ...
            "see 'authy users' and the sessions API.[/dim]",
            title="Sessions",
        )
    )
