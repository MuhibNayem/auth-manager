"""``authy config`` — CLI configuration persisted as JSON (§9).

Storage: ``~/.authy/config.json`` (override with ``--file`` or
``AUTHY_CONFIG_FILE``). Files are written ``0600`` (§0.8); values whose key
...
def config_cmd():
    """Manage the local Authy CLI configuration (~/.authy/config.json, 0600)."""
...
def show(config_file: Optional[str], reveal: bool):
    ...
    if not config:
        ...
        console.print("[dim]Use 'authy config set KEY VALUE' to create it.[/dim]")
        return
