"""``authy init`` — project scaffolding with REAL templates (CONTRACTS.md §9).

Generated artifacts use only real import paths (``authy_package.frameworks``
adapters and their §10 parity API: ``require_auth`` / ``require_role`` /
``rate_limit`` / ``optional_auth``). The generated ``.env`` receives a fresh
``AUTHY_JWT_SECRET`` from ``secrets.token_urlsafe(48)`` — never a
placeholder — and no hardcoded passwords are written anywhere.

An ``authy_migrations/`` directory with one sample migration is included so
``authy migrate`` works out of the box.
"""
...
DB_URL_TEMPLATES = {
    "postgresql": "postgresql+asyncpg://USER:PASSWORD@localhost:5432/authy",
    "mysql": "mysql+aiomysql://USER:PASSWORD@localhost:3306/authy",
    "sqlite": "sqlite+aiosqlite:///./authy.db",
...
def init_cmd(name: Optional[str], framework: Optional[str], database: Optional[str],
    ...
    """Initialize a new Authy project (honest, runnable scaffolding)."""
    ...
    console.print("  2. review [bold].env[/bold] (a real AUTHY_JWT_SECRET was generated)")
    console.print("  3. [bold]pip install -r requirements.txt[/bold]")
    console.print("  4. [bold]authy migrate run[/bold] then [bold]authy dev[/bold]")
...
def _scaffold(project_dir: Path, project_name: str, framework: str,
    ...
    for sub in ("src", "src/config", "authy_migrations", "tests"):
...
def _write_env(project_dir: Path, database: str) -> None:
    ...
    env_lines = [
        "# Generated by `authy init` — rotate any value you suspect was exposed.",
        "AUTHY_ENV=development",
        f"AUTHY_JWT_SECRET={jwt_secret}",
        f"AUTHY_DB_TYPE={db_type}",
    ]
    if db_url:
        env_lines.append(
            f"# Replace USER:PASSWORD with real credentials (never commit them):\n"
            f"AUTHY_DB_URL={db_url}"
        )
    else:
        env_lines.append("# DynamoDB uses the standard AWS credential chain:\nAWS_REGION=us-east-1")
    env_lines.append("AUTHY_CACHE_ENABLED=false  # enable + set AUTHY_REDIS_URL when ready")
...
def _write_config_module(project_dir: Path, project_name: str) -> None:
    ...
    content = f'''"""Authy configuration for {project_name} (generated by `authy init`)."""

import asyncio

from authy_package import AuthConfig, AuthManager
...
def _write_app_files(project_dir: Path, framework: str) -> None:
    ...
    if framework == "fastapi":
        content = '''"""FastAPI application wired to Authy (generated by `authy init`)."""
...
'''
        ...
        content = '''"""Flask application wired to Authy (generated by `authy init`)."""

from flask import Flask, jsonify, request

# Real import path (CONTRACTS.md §10):
from authy_package.frameworks.flask_adapter import FlaskAuth
...
'''
        ...
        content = '''"""Django views wired to Authy (generated by `authy init`).
...
``request.authy_user`` (Django's ``request.user`` is left untouched).
"""
...
# Real import path (CONTRACTS.md §10):
from authy_package.frameworks.django_adapter import DjangoAuth
...
def protected_route(request):
    ...
    user = getattr(request, "authy_user", None) or {}
...
'''
        ...
        content = '''"""Generic application wired to Authy (generated by `authy init`)."""
...
def _write_sample_migration(project_dir: Path) -> None:
    """authy_migrations/ with one real sample migration (CONTRACTS.md §9)."""
    content = '''"""Sample migration created by `authy init`.
...
async def upgrade(db):
    ...
    await db.set_setting("authy_sample_migrated", True)


async def downgrade(db):
    """Revert the sample migration."""
    await db.set_setting("authy_sample_migrated", None)
'''
    (project_dir / "authy_migrations" / "0001_sample.py").write_text(
...
def _write_requirements(project_dir: Path, framework: str, database: str,
    ...
    deps = ["authy-package>=2.0.0", "rich>=13.0.0", "click>=8.0.0", "python-dotenv>=1.0.0"]
    if framework == "fastapi":
...
Authentication application scaffolded by `authy init`.
...
authy doctor          # verify configuration
authy migrate run     # apply authy_migrations/
authy dev             # start the development server (127.0.0.1)
```

The `.env` file contains a freshly generated `AUTHY_JWT_SECRET`
