"""``authy migrate`` — real migrations against the §4 db contract (§9).

- Migration modules are discovered in ``./authy_migrations/`` (sorted by
...
MIGRATIONS_DIR_NAME = "authy_migrations"
...
def load_migration_module(path: Path) -> Any:
    ...
    spec = importlib.util.spec_from_file_location(
        f"authy_migration_{path.stem}", path
    )
...
def migrate_cmd():
    """Manage database migrations (./authy_migrations/, db-tracked)."""


@migrate_cmd.command()
@click.option("--name", "-n", required=True, help="Migration name")
def create(name: str):
    """Create a new migration module in ./authy_migrations/."""
...
async def _run(target: Optional[str], dry_run: bool) -> int:
    if not migrations_dir().exists():
        console.print(
            ...
            f"{Path.cwd()} (run 'authy migrate create -n NAME' first)[/red]"
        )
        return 1
