...
- :class:`~authy_package.admin.deps.AdminDependencies` on ``app.state``;
...
- ``POST /auth/login`` issues real JWTs through
  :class:`~authy_package.utils.security.JWTTokenManager`, enforces the §3.1
...
- AuthyError → HTTP status mapping is registered by
...
logger = logging.getLogger("authy.admin.dashboard")
...
async def admin_login(
    ...
    await deps.cache.set(
        f"authy:refresh:{pair['refresh_jti']}",
        user["id"],
...
async def admin_refresh(
    ...
    user_id = payload["sub"]
    old_jti = payload["jti"]
    stored = await deps.cache.get(f"authy:refresh:{old_jti}")
    if stored != user_id:
    ...
    await deps.cache.delete(f"authy:refresh:{old_jti}")
...
def install_admin_api(app: Any, deps: AdminDependencies) -> Any:
    ...
    - installs AuthyError/ValueError/PermissionError handlers (§1);
    - includes the v1 dashboard router, the v2 enterprise router and the
      RBAC router.
    """
    from authy_package.admin.dashboard_enterprise import enterprise_router
    from authy_package.admin.rbac_api import rbac_router
