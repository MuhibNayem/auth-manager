...
from authy_package.errors import AuthyError, RateLimitError, TokenError

logger = logging.getLogger("authy.frameworks.flask")
...
def _map_authy_error(exc: AuthyError):
    """Map AuthyError subclasses to (code, message, status)."""
    ...
    def require_auth(self, view: Callable) -> Callable:
        ...
        def decorated(*args, **kwargs):
            ...
            try:
                ...
                return _error_response(*_map_authy_error(exc))
    ...
    def optional_auth(self, view: Callable) -> Callable:
        ...
        def decorated(*args, **kwargs):
            ...
            if token:
                ...
                except (AuthyError, Exception):  # noqa: BLE001 - optional path
