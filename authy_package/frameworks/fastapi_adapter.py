...
- AuthyError subclasses map uniformly to HTTP status codes (§1).
...
logger = logging.getLogger("authy.frameworks.fastapi")

__all__ = ["FastAPIAuth", "authy_error_to_http_exception", "install_error_handlers"]

#: Rate-limit cache key (§3.1 schema: authy:ratelimit:{scope}:{id}).
_RATE_LIMIT_KEY_TEMPLATE = "authy:ratelimit:http:{identifier}"


def authy_error_to_http_exception(error: AuthyError) -> HTTPException:
    """Map an AuthyError to the HTTPException required by §1."""
...
def install_error_handlers(app: Any) -> None:
    """Register AuthyError/ValueError handlers on a FastAPI app."""
    from fastapi.responses import JSONResponse

    async def _authy_handler(request: Request, exc: AuthyError):
        http_exc = authy_error_to_http_exception(exc)
        return JSONResponse(
    ...
    app.add_exception_handler(AuthyError, _authy_handler)
...
class FastAPIAuth:
    ...
    async def require_auth(
        ...
        try:
            ...
            raise authy_error_to_http_exception(exc) from exc
    ...
    def rate_limit(self, max_requests: int, window_seconds: int):
        ...
        Uses cache counters under the §3.1 ``authy:ratelimit:`` namespace.
