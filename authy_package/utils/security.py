"""Security primitives for the Authy package (CONTRACTS.md §6).
...
- Async rate-limit / lockout helpers backed by cache counters
  (``authy:ratelimit:login:{identifier}`` / ``authy:lockout:{identifier}``).
- :class:`SecurityManager` implementing the password-reset flow: tokens from
  ...
  ``authy:reset:{token_hash}``, single-use atomic consume, constant-time
...
logger = logging.getLogger("authy.utils.security")
...
RESET_TOKEN_CACHE_PREFIX = "authy:reset:"

#: Cache key templates for login rate limiting / lockout (§3.1).
LOGIN_RATE_LIMIT_KEY_TEMPLATE = "authy:ratelimit:login:{identifier}"
LOCKOUT_KEY_TEMPLATE = "authy:lockout:{identifier}"
...
def _argon2_hasher():
    ...
    try:
        ...
        raise ImportError(
            ...
            "install authy-package with the corresponding extra"
        ) from exc
...
async def record_login_failure(
    ...
    *,
    ...
    The counter lives at ``authy:ratelimit:login:{identifier}`` with a
    ...
    at ``authy:lockout:{identifier}`` for ``account_lockout_duration_seconds``
...
class SecurityManager:
    ...
    only as sha256 hashes under ``authy:reset:{token_hash}``. Tokens are
