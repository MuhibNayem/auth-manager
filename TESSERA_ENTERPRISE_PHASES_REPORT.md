# Tessera Enterprise Phases 2–4 — Status Report (2.0 remediation)
...
while `TESSERA_ADMIN_DASHBOARD_REPORT.md` listed the same phases as TODO.
...
- API keys: create returns plaintext **once**; only the sha256 hash,
  ...
  `Authorization: Bearer tessera_ak_...` against the db on every
  `/admin/v2` route (§8).
