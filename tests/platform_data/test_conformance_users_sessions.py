"""Conformance: users CRUD, case-insensitive lookups, sessions incl. races."""
...
class TestUsers:
    async def test_create_defaults_and_tz_timestamps(
        ...
        created = to_datetime(user["created_at"])
        assert created.tzinfo is not None
        assert created.utcoffset() == timezone.utc.utcoffset(None)

    async def test_get_by_id_and_case_insensitive_identifiers(
        ...
        assert (
            await db.get_user_by_identifier(email="alice@example.TEST")
        )["id"] == created["id"]
        assert (
            await db.get_user_by_identifier(username="aLiCe")
        )["id"] == created["id"]
        assert (
            await db.get_user_by_identifier(phone="+15550001")
        )["id"] == created["id"]
        assert await db.get_user_by_identifier(email="missing@x.test") is None

    async def test_duplicate_identifiers_rejected_case_insensitively(
        self, db: AbstractDatabase
    ) -> None:
        ...
        with pytest.raises(IntegrityError):
            await db.create_user({"username": "U1", "email": "a2@x.test"})

    async def test_update_reindexes_and_rejects_immutable(
        self, db: AbstractDatabase
    ) -> None:
        ...
        with pytest.raises(ValueError):
            await db.update_user(user["id"], {"created_at": "evil"})
        ...
        assert (
            await db.get_user_by_identifier(email="NEW@X.TEST")
        )["id"] == user["id"]
        assert await db.update_user("missing", {"role": "admin"}) is None

    async def test_update_conflicting_email_rejected(
        self, db: AbstractDatabase
    ) -> None:
    ...
    async def test_delete_user(self, db: AbstractDatabase) -> None:
    ...
    async def test_list_users_pagination_search_filters(
        self, db: AbstractDatabase
    ) -> None:
        for i in range(7):
        ...
        with pytest.raises(ValueError):
            await db.list_users(limit=-1)

    async def test_count_users_filters(self, db: AbstractDatabase) -> None:
        for i in range(3):
            await db.create_user(
                {
                    "username": f"u{i}",
        ...
        assert await db.count_users(mfa_enabled=False) == 2
        future = datetime(2999, 1, 1, tzinfo=timezone.utc)
        assert await db.count_users(created_since=future) == 0


class TestSessions:
    async def test_save_requires_user_id(self, db: AbstractDatabase) -> None:
        with pytest.raises(ValueError):
            await db.save_session({"id": "s1"})

    async def test_save_get_and_extras(self, db: AbstractDatabase) -> None:
    ...
    async def test_revoke_session_atomic_flip(self, db: AbstractDatabase) -> None:
        await db.save_session({"id": "s1", "user_id": "u1", "status": "active"})
        assert await db.revoke_session("s1") is True
        assert await db.revoke_session("s1") is False
        assert await db.revoke_session("missing") is False
        session = await db.get_session("s1")
        assert session["status"] == "revoked"
        assert "revoked_at" in session

    async def test_concurrent_revokes_single_winner(
        self, db: AbstractDatabase
    ) -> None:
        await db.save_session({"id": "s1", "user_id": "u1", "status": "active"})
        ...
        assert (await db.get_session("s1"))["status"] == "revoked"

    async def test_revoke_all_user_sessions(self, db: AbstractDatabase) -> None:
        await db.save_session({"id": "s1", "user_id": "u1", "status": "active"})
...
class TestLifecycle:
    async def test_health_check(self, db: AbstractDatabase) -> None:
