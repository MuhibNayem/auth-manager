...
class TestFromEnv:
    def test_defaults_without_env(self) -> None:
        ...
        assert cfg.rp_name == "Tessera"
        assert cfg.email_provider == "mailjet"

    def test_legacy_tessera_env_names_preserved(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("TESSERA_ENV", "staging")
        monkeypatch.setenv("TESSERA_JWT_SECRET", "s" * 40)
        monkeypatch.setenv("TESSERA_DB_TYPE", "memory")
        monkeypatch.setenv("TESSERA_DB_NAME", "tessera")
        monkeypatch.setenv("TESSERA_TOKEN_EXPIRATION", "1800")
        monkeypatch.setenv("TESSERA_REFRESH_TOKEN_EXPIRATION", "1234")
        monkeypatch.setenv("TESSERA_RATE_LIMIT_ENABLED", "false")
        monkeypatch.setenv("TESSERA_RATE_LIMIT_MAX_ATTEMPTS", "9")
        monkeypatch.setenv("TESSERA_REDIS_URL", "redis://cache:6380")
        monkeypatch.setenv("TESSERA_CACHE_ENABLED", "true")
        monkeypatch.setenv("TESSERA_EMAIL_ENABLED", "true")
        monkeypatch.setenv("MAILJET_API_KEY", "mj-key")
        monkeypatch.setenv("MAILJET_API_SECRET", "mj-secret")
        monkeypatch.setenv("SENDER_EMAIL", "noreply@tessera.test")
        monkeypatch.setenv("SENDER_NAME", "Tessera NoReply")

        cfg = AuthConfig.from_env()
        assert cfg.env == "staging"
        assert cfg.jwt_secret == "s" * 40
        assert cfg.database.db_type == "memory"
        assert cfg.database.db_name == "tessera"
        ...
        assert cfg.sender_email == "noreply@tessera.test"
        assert cfg.sender_name == "Tessera NoReply"

    def test_new_canonical_env_names(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("TESSERA_JWT_ISSUER", "https://issuer.tessera.test")
        monkeypatch.setenv("TESSERA_JWT_AUDIENCE", "tessera-api")
        monkeypatch.setenv("TESSERA_JWT_CLOCK_SKEW_SECONDS", "45")
        monkeypatch.setenv("TESSERA_SESSION_EXPIRY_SECONDS", "3600")
        monkeypatch.setenv("TESSERA_MAX_CONCURRENT_SESSIONS", "2")
        monkeypatch.setenv("TESSERA_REVOKE_SESSIONS_ON_PASSWORD_CHANGE", "false")
        monkeypatch.setenv("TESSERA_RATE_LIMIT_WINDOW_SECONDS", "60")
        monkeypatch.setenv("TESSERA_ACCOUNT_LOCKOUT_DURATION_SECONDS", "120")
        monkeypatch.setenv("TESSERA_MFA_REQUIRED", "true")
        monkeypatch.setenv("TESSERA_PASSWORD_HASH_ALGORITHM", "argon2")
        monkeypatch.setenv("TESSERA_BCRYPT_ROUNDS", "10")
        monkeypatch.setenv("TESSERA_RESET_TOKEN_TTL_SECONDS", "300")
        monkeypatch.setenv("TESSERA_BASE_URL", "https://auth.tessera.test")
        monkeypatch.setenv("TESSERA_MAGIC_LINK_TTL_SECONDS", "120")
        monkeypatch.setenv("TESSERA_DEFAULT_REDIRECT_URL", "https://auth.tessera.test/home")
        monkeypatch.setenv("TESSERA_AUTO_CREATE_USERS", "true")
        monkeypatch.setenv("TESSERA_RP_ID", "tessera.test")
        monkeypatch.setenv("TESSERA_RP_NAME", "Tessera Test")
        monkeypatch.setenv("TESSERA_REFRESH_TOKEN_TTL_DAYS", "14")

        cfg = AuthConfig.from_env()
        assert cfg.jwt_issuer == "https://issuer.tessera.test"
        assert cfg.jwt_audience == "tessera-api"
        ...
        assert cfg.base_url == "https://auth.tessera.test"
        assert cfg.magic_link_ttl_seconds == 120
        assert cfg.default_redirect_url == "https://auth.tessera.test/home"
        assert cfg.auto_create_users is True
        assert cfg.rp_id == "tessera.test"
        assert cfg.rp_name == "Tessera Test"
    ...
    def test_malformed_int_env_raises_config_error(
        ...
        monkeypatch.setenv("TESSERA_TOKEN_EXPIRATION", "not-a-number")
...
class TestValidate:
    ...
    def test_production_http_base_url_rejected(self, make_config) -> None:
        cfg = make_config(env="production", base_url="http://auth.tessera.test")
    ...
    def test_production_placeholder_mailjet_key_rejected(self, make_config) -> None:
        cfg = make_config(
            env="production",
            base_url="https://auth.tessera.test",
    ...
    def test_valid_production_config_passes(self, make_config) -> None:
        cfg = make_config(
            ...
            sender_email="noreply@tessera.test",
        )
        assert cfg.validate() is True
